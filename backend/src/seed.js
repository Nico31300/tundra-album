...
const ALBUMS = [
  ...
  },
  {
    ...
    puzzles: [
      ...
      { name: "Call of the Stars", pieces: 18 },
    ],
  },
  {
  ...
  },
  {
    ...
    puzzles: [
      ...
      { name: "Epic of a New World", pieces: 18 },
    ],
  },
  {
    name: "The Labyrinth",
...
function getPieceStars(position, total, fiveStarCount) {
  if (position > total - fiveStarCount) return 5;
  const remaining = total - fiveStarCount;
  if (remaining === 0) return 5;
  const maxStars = total >= 15 ? 4 : Math.min(4, Math.ceil(total / 3));
  return Math.ceil((position / remaining) * maxStars);
}
...
const seedAll = db.transaction(() => {
  ...
  for (const [albumIdx, album] of ALBUMS.entries()) {
    ...
    const bigPuzzles = album.puzzles.filter((p) => p.pieces >= 15);
    ...
    for (const [puzzleIdx, puzzle] of album.puzzles.entries()) {
      ...
      const fiveStarCount = bigPuzzles.indexOf(puzzle) + 1 || 0;
      ...
      for (let pc = 1; pc <= puzzle.pieces; pc++) {
        insertPiece.run(
          ...
          String(pc),
          pc,
          getPieceStars(pc, puzzle.pieces, fiveStarCount),
        );
      }
